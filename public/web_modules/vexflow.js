...
function L$1(...args) { if (NoteHead.DEBUG) Vex.L('Vex.Flow.NoteHead', args); }
...
class NoteHead extends Note {
  ...
  draw() {
    ...
    L$1("Drawing note head '", this.note_type, this.duration, "' at", head_x, y);
...
class Modifier extends Element {
...
function L$2(...args) { if (StaveNote.DEBUG) Vex.L('Vex.Flow.StaveNote', args); }
...
function L$3(...args) { if (Accidental.DEBUG) Vex.L('Vex.Flow.Accidental', args); }
...
class Accidental extends Modifier {
  ...
  static format(accidentals, state) {
    ...
    lineList.forEach(line => {
      ...
      for (accCount; accCount < lastAccOnLine; accCount++) {
        ...
        L$3('Line, accCount, shift: ', line.line, accCount, xShift);
      }
    });
  ...
  static checkCollision(line1, line2) {
    ...
    L$3('Line_1, Line_2, Collision: ', line1.line, line2.line, collision);
    return collision;
  }
...
function L$4(...args) { if (GraceNoteGroup.DEBUG) Vex.L('Vex.Flow.GraceNoteGroup', args); }
...
class GraceNoteGroup extends Modifier {
  ...
  draw() {
    ...
    L$4('Drawing grace note group for:', note);
...
function L$5(...args) { if (Articulation.DEBUG) Vex.L('Vex.Flow.Articulation', args); }
...
class Articulation extends Modifier {
  ...
  draw() {
    ...
    L$5(`Rendering articulation at (x: ${x}, y: ${y})`);
...
class TickContext extends Tickable {
...
function L$6(...args) { if (Ornament.DEBUG) Vex.L('Vex.Flow.Ornament', args); }
...
class Ornament extends Modifier {
  ...
  draw() {
    ...
    L$6('Rendering ornament: ', this.ornament, glyphX, glyphY);
...
function L$7(...args) { if (Annotation.DEBUG) Vex.L('Vex.Flow.Annotation', args); }
...
class Annotation extends Modifier {
  ...
  draw() {
    ...
    L$7('Rendering annotation: ', this.text, x, y);
...
function L$8(...args) { if (ModifierContext.DEBUG) Vex.L('Vex.Flow.ModifierContext', args); }
...
class ModifierContext {
  ...
  preFormat() {
    ...
    this.PREFORMAT.forEach((modifier) => {
      L$8('Preformatting ModifierContext: ', modifier.CATEGORY);
  ...
  postFormat() {
    ...
    this.POSTFORMAT.forEach((modifier) => {
      L$8('Postformatting ModifierContext: ', modifier.CATEGORY);
...
function L$9(...args) { if (Formatter.DEBUG) Vex.L('Vex.Flow.Formatter', args); }
...
class Formatter {
  ...
  formatToStave(voices, stave, options) {
    ...
    L$9('Formatting voices to width: ', justifyWidth);
...
function L$a(...args) { if (Clef.DEBUG) Vex.L('Vex.Flow.Clef', args); }
...
function L$b(...args) { if (VibratoBracket.DEBUG) Vex.L('Vex.Flow.VibratoBracket', args); }
...
class VibratoBracket extends Element {
  ...
  draw() {
    ...
    L$b('Rendering VibratoBracket: start_x:', start_x, 'stop_x:', stop_x, 'y:', y);
...
function L$c(...args) { if (TextDynamics.DEBUG) Vex.L('Vex.Flow.TextDynamics', args); }
...
class TextDynamics extends Note {
  ...
  constructor(text_struct) {
    ...
    L$c('New Dynamics Text: ', this.sequence);
  }
  ...
  draw() {
    ...
    L$c('Rendering Dynamics: ', this.sequence);
...
function L$d(...args) { if (PedalMarking.DEBUG) Vex.L('Vex.Flow.PedalMarking', args); }
...
function L$e(...args) { if (TextBracket.DEBUG) Vex.L('Vex.Flow.TextBracket', args); }
...
class TextBracket extends Element {
  ...
  draw() {
    ...
    L$e('Rendering TextBracket: start:', start, 'stop:', stop, 'y:', y);
...
function L$f(...args) { if (BarNote.DEBUG) Vex.L('Vex.Flow.BarNote', args); }
...
class BarNote extends Note {
  ...
  draw() {
    this.checkContext();
    if (!this.stave) throw new Vex.RERR('NoStave', "Can't draw without a stave.");
    L$f('Rendering bar line at: ', this.getAbsoluteX());
...
function L$g(...args) { if (Crescendo.DEBUG) Vex.L('Vex.Flow.Crescendo', args); }
...
function L$h(...args) { if (Parser.DEBUG) Vex.L('Vex.Flow.Parser', args); }

const X$1 = Vex.MakeException('ParserError');
...
class Parser {
  ...
  expect(rules) {
    L$h('Evaluating rules:', rules);
    let result;
    if (!rules) {
      throw new X$1('Invalid Rule: ' + rules, rules);
    }
    ...
    if (rule.token) {
      ...
      throw new X$1('Bad grammar! No `token` or `expect` property', rule);
    }
...
function L$i(...args) { if (EasyScore.DEBUG) Vex.L('Vex.Flow.EasyScore', args); }

const X$2 = Vex.MakeException('EasyScoreError');
...
class Builder {
  ...
  setNoteDots(dots) {
    L$i('setNoteDots:', dots);
    if (dots) this.piece.dots = dots.length;
  }

  setNoteDuration(duration) {
    L$i('setNoteDuration:', duration);
    this.rollingDuration = this.piece.duration = duration || this.rollingDuration;
  }

  setNoteType(type) {
    L$i('setNoteType:', type);
    if (type) this.piece.type = type;
  }

  addNoteOption(key, value) {
    L$i('addNoteOption: key:', key, 'value:', value);
    this.piece.options[key] = value;
  }

  addNote(key, accid, octave) {
    L$i('addNote:', key, accid, octave);
    this.piece.chord.push({ key, accid, octave });
  }

  addSingleNote(key, accid, octave) {
    L$i('addSingleNote:', key, accid, octave);
...
class EasyScore {
  ...
  parse(line, options = {}) {
    ...
    if (!result.success && this.options.throwOnError) {
      throw new X$2('Error parsing line: ' + line, result);
    }
    return result;
  }
...
function L$j(...args) { if (Factory.DEBUG) Vex.L('Vex.Flow.Factory', args); }

const X$3 = Vex.MakeException('FactoryError');
...
class Factory {
  ...
  initRenderer() {
    ...
    if (elementId === '') {
      throw new X$3('HTML DOM element not set in Factory');
    }
...
function setDefaults$1(params, defaults) {
  const default_options = defaults.options;
  params = Object.assign(defaults, params);
  params.options = Object.assign(default_options, params.options);
  return params;
...
class System extends Element {
  ...
  setOptions(options = {}) {
    this.options = setDefaults$1(options, {
      x: 10,
      y: 10,
      width: 500,
      connector: null,
